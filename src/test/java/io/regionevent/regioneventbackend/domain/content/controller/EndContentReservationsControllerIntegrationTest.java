...
class EndContentReservationsControllerIntegrationTest extends ContentControllerWebMvcTestSupport {
    ...
    void 콘텐츠_예약_종료_유효한_요청이면_응답을_직렬화한다() throws Exception {
        when(endContentReservationsUseCase.endByRegionAdmin(
            ...
            eq(CONTENT_ID),
            any(UUID.class)
        )).thenReturn(new EndContentReservationsResult(
            CONTENT_ID,
            ContentStatus.ENDED,
            Instant.parse("2026-08-04T12:00:00Z")
        ));

        mockMvc.perform(authenticated(post("/api/v1/region-admin/contents/{contentId}/end", CONTENT_ID)))
    ...
    @Test
    void 콘텐츠_예약_종료_인증이_없으면_유스케이스를_호출하지_않는다() throws Exception {
        mockMvc.perform(post("/api/v1/region-admin/contents/{contentId}/end", CONTENT_ID))
    ...
    @Test
    void 콘텐츠_예약_종료_경로_ID가_양의_정수가_아니면_입력_오류를_반환한다() throws Exception {
        mockMvc.perform(authenticated(post("/api/v1/region-admin/contents/0/end")))
            .andExpect(status().isBadRequest())
            .andExpect(jsonPath("$.code").value("INVALID_INPUT"));

        mockMvc.perform(authenticated(post("/api/v1/region-admin/contents/not-a-number/end")))
            .andExpect(status().isBadRequest())
            .andExpect(jsonPath("$.code").value("INVALID_INPUT"));
    }

    @Test
    void 콘텐츠_예약_종료_관할_불일치는_권한_오류로_응답한다() throws Exception {
        expectBusinessError(ErrorCode.FORBIDDEN, 403, "FORBIDDEN");
    }

    @Test
    void 콘텐츠_예약_종료_상태_충돌은_공통_오류로_응답한다() throws Exception {
