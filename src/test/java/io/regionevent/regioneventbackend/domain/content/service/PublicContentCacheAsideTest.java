...
class PublicContentCacheAsideTest {
    ...
    @Test
    void resolveContent_현재_버전_키의_캐시가_있으면_정적_표시_정보_조회_없이_재사용한다() {
        PublicContentStaticInfo cached = content(3, "캐시 제목");
        @SuppressWarnings("unchecked")
        Supplier<PublicContentStaticInfo> staticInfoLoader = mock(Supplier.class);
        when(publicContentCache.findContent(10L, 200L, 3)).thenReturn(Optional.of(cached));

        PublicContentStaticInfo result = cacheAside.resolveContent(10L, 200L, 3, staticInfoLoader);

        assertThat(result).isEqualTo(cached);
        verifyNoInteractions(staticInfoLoader);
        verify(publicContentCache, never()).saveContent(cached);
    }

    @Test
    void resolveContent_이전_버전_캐시가_반환되면_정적_표시_정보를_조회하고_현재_키에_저장한다() {
        ...
        PublicContentStaticInfo result = cacheAside.resolveContent(10L, 200L, 4, staticInfoLoader);
