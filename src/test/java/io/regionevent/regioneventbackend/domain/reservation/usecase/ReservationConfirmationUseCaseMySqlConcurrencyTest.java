...
class ReservationConfirmationUseCaseMySqlConcurrencyTest {
    ...
    @Autowired
    ReservationConfirmationUseCaseMySqlConcurrencyTest(
    ...
    @Test
    void claimProcessing_두_요청이_선조회_뒤_경합하면_기존_기록은_신규_점유로_판정하지_않는다() throws Exception {
        ReservationFixtures fixtures = createFixtures();
        String idempotencyKeyHash = UUID.randomUUID().toString().replace("-", "");
        CountDownLatch prechecked = new CountDownLatch(CONCURRENT_REQUEST_COUNT);
        ...
        try {
            Future<Integer> first = executorService.submit(() -> jdbcTemplate.execute((ConnectionCallback<Integer>) connection -> {
                connection.setAutoCommit(false);
                assertThat(countIdempotencyRecords(connection, fixtures.user().getUserId(), idempotencyKeyHash)).isZero();
                prechecked.countDown();
                await(start);
                int insertedCount = insertProcessingRecord(connection, fixtures.user().getUserId(), idempotencyKeyHash);
                firstInserted.countDown();
                await(secondInsertStarted);
                connection.commit();
                return insertedCount;
            }));
            Future<Integer> second = executorService.submit(() -> jdbcTemplate.execute((ConnectionCallback<Integer>) connection -> {
                connection.setAutoCommit(false);
                assertThat(countIdempotencyRecords(connection, fixtures.user().getUserId(), idempotencyKeyHash)).isZero();
                prechecked.countDown();
                await(start);
            ...
            assertThat(prechecked.await(CONCURRENT_REQUEST_TIMEOUT_SECONDS, TimeUnit.SECONDS)).isTrue();
            start.countDown();

            assertThat(first.get(CONCURRENT_REQUEST_TIMEOUT_SECONDS, TimeUnit.SECONDS)).isEqualTo(1);
            assertThat(second.get(CONCURRENT_REQUEST_TIMEOUT_SECONDS, TimeUnit.SECONDS)).isZero();
        } finally {
    ...
    private static int countIdempotencyRecords(Connection connection, Long userId, String idempotencyKeyHash)
        throws SQLException {
        try (PreparedStatement statement = connection.prepareStatement("""
            ...
            statement.setString(2, idempotencyKeyHash);
    ...
    private static int insertProcessingRecord(Connection connection, Long userId, String idempotencyKeyHash)
        throws SQLException {
        try (PreparedStatement statement = connection.prepareStatement("""
            INSERT IGNORE INTO idempotency_record (
                ...
                created_at,
                expires_at
            ) VALUES (?, 'RESERVATION_CONFIRM', ?, 'request-hash', 'PROCESSING', UTC_TIMESTAMP(), DATE_ADD(UTC_TIMESTAMP(), INTERVAL 1 DAY))
            """)) {
            statement.setLong(1, userId);
            statement.setString(2, idempotencyKeyHash);
    ...
    private static void await(CountDownLatch latch) {
        try {
            if (!latch.await(CONCURRENT_REQUEST_TIMEOUT_SECONDS, TimeUnit.SECONDS)) {
                throw new IllegalStateException("Concurrent idempotency claim did not complete");
            }
        } catch (InterruptedException exception) {
            Thread.currentThread().interrupt();
            throw new IllegalStateException("Interrupted while waiting for concurrent idempotency claim", exception);
        }
    }
