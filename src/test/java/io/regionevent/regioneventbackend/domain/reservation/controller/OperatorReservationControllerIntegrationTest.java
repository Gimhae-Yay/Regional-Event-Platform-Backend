...
@Transactional
public class OperatorReservationControllerIntegrationTest {
    ...
    @Autowired
    OperatorReservationControllerIntegrationTest(
        MockMvc mockMvc,
    ...
    void registerUnixTimestamp() {
        entityManager.createNativeQuery(
            "CREATE ALIAS IF NOT EXISTS UNIX_TIMESTAMP FOR \"io.regionevent.regioneventbackend.domain.reservation.controller.OperatorReservationControllerIntegrationTest.unixTimestamp\""
        ).executeUpdate();
    }
    ...
    @Test
    void search_소유_운영자가_정상_조회하면_마스킹된_결과와_감사를_반환한다() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/operator/reservations/search")
            ...
            .andExpect(jsonPath("$.data.participant.userId").doesNotExist())
            .andExpect(jsonPath("$.data.qrReference").doesNotExist())
            .andExpect(jsonPath("$.data.checkIn.checkedIn").value(false))
            .andExpect(jsonPath("$.data.checkIn.canCheckIn").value(true))
        ...
        assertThat(auditEventActorLinkRepository.findById(auditEvent.getAuditEventId()))
            .hasValueSatisfying(link ->
                assertThat(link.getActor().getUserId()).isEqualTo(fixture.operator().getUserId())
            );
    }

    @Test
    void search_다른_소유_운영자는_FORBIDDEN을_반환하고_감사를_남기지_않는다() throws Exception {
        Fixture fixture = createFixture(AppUserStatus.ACTIVE, ReservationStatus.CONFIRMED, false);
        AppUser otherOperator = saveOperator(fixture.region(), AppUserStatus.ACTIVE);
    ...
    @Test
    void search_다른_담당_지역_운영자는_FORBIDDEN을_반환하고_감사를_남기지_않는다() throws Exception {
        Fixture fixture = createFixture(AppUserStatus.ACTIVE, ReservationStatus.CONFIRMED, false);
        Region otherRegion = regionRepository.saveAndFlush(new Region("DONGHAE", "동해시", true));
    ...
    @Test
    void search_비활성_운영자는_FORBIDDEN을_반환하고_감사를_남기지_않는다() throws Exception {
        Fixture fixture = createFixture(AppUserStatus.WITHDRAWING, ReservationStatus.CONFIRMED, false);
    ...
    @Test
    void search_취소된_예약은_조회되지만_canCheckIn은_false다() throws Exception {
        Fixture fixture = createFixture(AppUserStatus.ACTIVE, ReservationStatus.CANCELLED, false);
    ...
    @Test
    void search_완료된_회차는_조회되지만_canCheckIn은_false다() throws Exception {
    ...
    @Test
    void search_공백_예약번호는_INVALID_INPUT을_반환한다() throws Exception {
        Fixture fixture = createFixture(AppUserStatus.ACTIVE, ReservationStatus.CONFIRMED, false);
    ...
    private Fixture createFixture(
        ...
        AppUser operator = saveOperator(region, operatorStatus);
        ...
        return new Fixture(region, operator, content, session, reservation);
    }

    private AppUser saveOperator(Region region, AppUserStatus status) {
        String suffix = Long.toUnsignedString(System.nanoTime());
        AppUser operator = saveUser("operator-" + suffix, "운영자", "010-9876-5432", status);
