    ...
    private Instant latestRefundUpdatedAt(Refund refund) {
        ...
        if (refund.getStatus() == RefundStatus.PROCESSING) {
            ...
            if (retryAttemptedAt != null) {
                updatedAt = latest(List.of(updatedAt, retryAttemptedAt));
            }
        }
        return updatedAt;
    }

    private Instant latestRetryAttemptedAt(Refund refund) {
        List<RefundAttempt> attempts = refundAttemptService.findAllByRefundId(refund.getRefundId());
        if (attempts.size() < 2) {
            return null;
        }
        return attempts.getLast().getAttemptedAt();
    }
