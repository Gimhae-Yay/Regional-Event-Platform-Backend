...
ADR-0118은 자동 종료 스케줄러와 수동 종료 API가 같은 결과를 만들도록 "공통 종료 서비스"를 호출한다고
...
- ADR-0119의 공통 `content` 행 잠금과 ADR-0120의 회차 종결 상태 기준은 바꾸지 않아야 한다.
...
응답한다. 이 상태 조건 거부나 처리 실패는 종료 트랜잭션이 롤백된 뒤
[ADR-0015](0015-store-audit-events-in-mysql-with-withdrawn-actor-anonymization.md#결정)에 따라 실패 감사만 별도
트랜잭션으로 기록한다. 자동 종료는 별도 사용자 권한을 확인하지 않고 시스템 처리자를 기록하며, 이미 종료됐거나
종결되지 않은 회차가 생긴 후보는 실패로 기록하지 않고 정상적으로 건너뛴다. 이 입력 경로의 차이를 제외한 잠금과
상태 변경 결과는 같다.
...
- 자동·수동 종료와 추가 회차 생성의 두 실행 순서에서 ADR-0119의 공통 잠금 규칙이 유지되는지 검증한다.
