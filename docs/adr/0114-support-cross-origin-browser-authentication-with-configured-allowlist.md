# ADR-0114: 단일 허용 Origin과 Origin 검증으로 교차 출처 브라우저 인증을 지원한다
...
공개 프런트엔드는 `https://local-stamp.org` 하나만 사용하고 API는 `https://api.local-stamp.org`에서 제공한다. 두 주소는
서로 다른 Origin이지만 같은 HTTPS site다. 허용 Origin을 넓게 해석하거나 와일드카드로 대체하면 credential Cookie가
있는 브라우저 인증 경계를 무너뜨린다. Stateless Bearer 업무 API와 Refresh Cookie의 경계를 유지한 채, 정확한 단일
Origin만 허용하고 Cookie 기반 인증 명령의 Origin을 서버에서 검증해야 한다.

## 결정 동인과 불변 조건

- 허용 Origin은 `https://local-stamp.org` 하나이며 환경 변수나 런타임 구성으로 변경하지 않는다. 와일드카드·정규식·경로·쿼리·fragment를 허용하지 않는다.
...
- `https://local-stamp.org` 이외의 Origin 또는 Origin 없는 인증 명령은 허용하지 않는다.

## 검토한 선택지

| 순서 | 선택지 | 장점 | 단점·실패 위험 | 되돌림 비용 | 현재 단계 적합성 |
| --- | --- | --- | --- | --- | --- |
| 1 | 단일 HTTPS Origin, credential CORS와 서버 Origin 검증 | 환경 변수의 오설정 없이 필요한 교차 Origin만 허용한다. 별도 Token 발급·저장·회전 API 없이 Cookie 기반 인증 명령을 보호하고 `SameSite=Strict`를 유지한다. | 프런트엔드 Origin을 바꾸려면 애플리케이션 변경·배포가 필요하다. | 낮음 | 추천안. 현재 `local-stamp.org` 단일 프런트엔드와 Stateless Bearer·제한된 Refresh Cookie 경계에 맞는다. |
| 2 | 환경별 allowlist와 구성 가능한 `SameSite` | 배포별 프런트엔드 Origin과 Cookie 정책을 코드 변경 없이 바꿀 수 있다. | 사용하지 않는 설정값이 보안 정책을 넓히거나 배포 실수로 바뀔 여지를 만든다. | 중간 | 단일 HTTPS 프런트엔드 정책에는 과도하다. |
| 3 | 동일 Origin 프록시만 유지 | Cookie와 CSRF 계약이 단순하다. | #1023의 서로 다른 Origin 배포 조건을 충족하지 못하며 배포 선택지를 제한한다. | 낮음 | 현재 요구사항에 맞지 않는다. |

## 결정

교차 출처 브라우저 요청의 유일한 허용 Origin은 `https://local-stamp.org`로 고정한다. 이 값은 환경 변수나 런타임 구성으로
변경하지 않는다.

구현은 요청 `Origin`이 `https://local-stamp.org`와 정확히 일치할 때만 해당 값을 `Access-Control-Allow-Origin`에 반영하고
...
CORS 헤더를 보내지 않으며, 사전 요청도 보안 인증보다 먼저 위 단일 Origin과 method·header만으로 판정한다.

Refresh Cookie의 `SameSite`는 `Strict`로 고정한다. 프런트엔드와 API가 같은 HTTPS site이므로 다른 Origin이어도 Cookie를
전송할 수 있다. `HttpOnly`, `Secure`, `Path=/api/v1/auth`, `Domain` 생략의 호스트 전용 경계와 14일 절대 만료는 유지한다.

Refresh Cookie를 수신하거나 사용하는 `POST /api/v1/auth/login`, `POST /api/v1/auth/refresh`,
`POST /api/v1/auth/logout`은 `Origin` header를 반드시 포함해야 한다. 서버는 그 값이 `https://local-stamp.org`와 정확히 일치할 때만
...
이 ADR은 정책과 공개 계약만 확정한다. `SecurityConfig`, CORS·Origin 검증 구현과 자동화
...
- `https://local-stamp.org`만 정확히 허용해 환경 변수 오설정으로 인증 경계가 바뀌지 않는다.
...
- 프런트엔드 Origin을 추가하거나 변경하려면 이 ADR을 대체하는 새 정책과 애플리케이션 배포가 필요하다. 운영 장애를 피하려고 와일드카드로 완화하지 않는다.
- 서버가 Origin 없는 브라우저 인증 명령을 허용하면 교차 사이트 form 요청을 구분할 수 없으므로, 호환성보다 fail-closed를 택한다.

## 전환과 롤백

문서 확정 뒤 별도 구현 Task에서 고정 Origin CORS 처리, 인증 명령의 Origin 검증, Refresh Cookie `SameSite=Strict` 설정과
프런트엔드 `credentials` 처리를 하나의 호환 배포로 추가한다. 구현 전에는 현재
`SameSite=Strict`·CORS 비활성 코드와 문서 계약이 일시적으로 다를 수 있으며, 이 ADR만으로 런타임 동작은 바뀌지 않는다.

배포 결함이나 의심스러운 Origin 확장이 확인되면 CORS 처리를 포함한 애플리케이션 배포를 롤백해 교차 출처 요청을 차단하고,
승인된 동일 Origin 경로만 사용한다. Origin 검증 결함은 검증을 생략하지 않고 해당 인증 명령 배포를 함께 롤백한다.

## 검증 방법

- 허용 Origin과 허용 method·header 조합의 `OPTIONS`가 정확한 `Access-Control-Allow-*`와 `Vary: Origin`을 반환하는지 검증한다.
- `https://local-stamp.org` 이외 Origin, 허용하지 않은 method·header가 CORS credential을 허용하지 않는지 검증한다.
- 허용 Origin의 로그인·재발급·로그아웃 요청만 성공하고, 다른 Origin·Origin 누락은 `403 FORBIDDEN`인지 검증한다.
- `https://local-stamp.org`에서 `SameSite=Strict` Refresh Cookie 송수신, 로그인 후 Bearer 보호 API 응답 읽기, 로그아웃 Cookie 만료를 실제 브라우저 E2E로 검증한다.
- 응답·로그에 Refresh Token 원문이 남지 않고, 보호 업무 API가 Refresh Cookie만으로 인증되지 않는지 검증한다.

## 대체 조건

- 프런트엔드 Origin을 추가하거나 변경해야 한다.
