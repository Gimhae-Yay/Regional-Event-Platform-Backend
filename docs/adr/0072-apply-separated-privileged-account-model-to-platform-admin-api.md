...
확정된 [P1 ERD](../p1-erd.md)는 일반 역할과 전체관리자 권한을 같은 역할 연결에 저장하지 않는다.
...
| 2 | `user_role_assignment`에 `PLATFORM_ADMIN`을 추가한다 | 기존 역할 조회를 재사용한다 | 확정 ERD 및 계정 유형 분리 규칙과 충돌한다 | 기각 |
