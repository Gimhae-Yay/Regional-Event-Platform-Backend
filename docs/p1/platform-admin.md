...
- `ADM-02`:
  ...
  비삭제 콘텐츠 조건 실패는 변경 롤백 뒤 서버 실패 코드와 검증된 요청 증빙 참조를 가진 실패 감사로 기록한다.
- `ADM-03`:
