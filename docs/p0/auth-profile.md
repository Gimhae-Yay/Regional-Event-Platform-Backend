...
| [ADR-0116](../adr/0116-use-latest-operator-application-for-status-and-reapplication.md#결정) | 본인 신청 현황 조회와 재신청 허용 여부의 최신 신청 단일 기준 |

### 기능 범위

- 지역 관리자는 신청자의 사업자 정보와 요청 지역을 수동 확인한 뒤 운영자를 승인·반려한다.
- 인증된 회원은 신청자 연결이 유지된 본인의 가장 최근 운영자 신청 상태와 비민감 심사 결과를 조회한다.
...
신청자 연결이 유지된 본인의 신청은 `created_at` 내림차순, 같은 시각이면 `operator_application_id` 내림차순으로
정렬한 첫 행을 가장 최근 신청으로 판정한다. 본인 신청 현황 조회는 이 신청의 식별자·상태·요청 지역·신청 시각·
심사 시각·반려 사유만 반환하고 사업자 정보 원문과 심사자 정보는 반환하지 않는다. 신청 이력이 없으면 신청 객체를
`null`로 반환한다. 회원 탈퇴로 `CANCELLED`된 신청은 신청자 연결과 계정이 함께 제거되므로 현재 본인 조회 응답에는
노출하지 않는다.

반려 뒤 다시 신청하려면 새 신청을 생성하며, 승인 전 신청자는 운영자 기능을 사용할 수 없다. 재신청 제출은 회원 행을
잠근 쓰기 트랜잭션에서 가장 최근 신청 상태를 다시 확인한다. 가장 최근 신청이 `REJECTED`일 때만 새 `PENDING` 신청을
생성하고, `PENDING`이면 처리 중 오류로 거부하며, 신청 이력이 없거나 가장 최근 신청이 `APPROVED` 또는 `CANCELLED`이면
재신청 불가 오류로 거부한다. 이미 `OPERATOR` 또는 `REGION_ADMIN` 역할이 부여된 회원을 거부하는 기존 인가 경계는
유지하며, 역할 검사를 가장 최근 신청 상태 검사보다 먼저 적용한다.
