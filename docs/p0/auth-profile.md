...
| [ADR-0114](../adr/0114-support-cross-origin-browser-authentication-with-configured-allowlist.md#결정) | 환경별 교차 출처 CORS allowlist, Refresh Cookie와 Origin 검증 경계 |
...
- 로그인 성공 시 Access Token은 JSON 본문의 `data.accessToken`으로, 14일 절대 만료 Stateless Refresh Token은 `HttpOnly`·`Secure`·`SameSite=Strict` Cookie로 발급한다. 환경별 허용 Origin, credential과 Origin 검증 규칙은 [인증·인가 공통 계약](../api/common/authentication.md#교차-출처-corscookieorigin-검증-계약)을 따른다. Access Token 재발급 성공 시에는 Access Token만 JSON 본문으로 반환하고 Refresh Token 쿠키를 교체하지 않는다.
