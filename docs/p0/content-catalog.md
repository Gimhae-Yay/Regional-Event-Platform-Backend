...
소유 운영자는 콘텐츠 식별자로 가장 큰 `revision_no`를 가진 최신 수정본 상세를 조회할 수 있어, 별도 수정본 식별자를
보관하지 않아도 새로고침이나 다른 브라우저에서 작업을 이어 갈 수 있어야 한다. 운영자용 최신 수정본 상세는 현재 원본
값과 병합하지 않은 후보 필드, 후보 대표 이미지 조회 URL과 상태별 반려 사유·처리 시각을 반환하며 이미지 객체 식별자와
객체 키는 노출하지 않는다. 기존 내 콘텐츠 상세 응답에는 수정본 요약을 추가하지 않는다.

가장 최근 수정본이 `EDIT_REJECTED`이면 소유 운영자는 저장된 후보를 편집한 뒤 재제출할 수 있다. 재제출은 기존
반려 수정본의 상태를 되돌리지 않고 모든 후보 필드, 후보 `reservation_price`, 후보 `publish_at`, 후보 대표 이미지
객체와 연결 시각을 새 `EDIT_REQUESTED` 수정본으로 복제한다. 후보 이미지는 이미 연결된 객체이므로 신규 업로드 연결
검증을 다시 적용하거나 클라이언트에 이미지 객체 식별자 재전송을 요구하지 않는다. 원본 콘텐츠와 반려 수정본을 잠근 뒤
최신 수정본 여부, 원본 상태·후보 `publish_at`, 활성 수정본 부재와 소유·지역 경계를 검증하며, 일반 수정본 생성 또는
다른 재제출과 경합하면 하나만 새 활성 수정본을 생성한다. 성공한 재제출을 반복하면 활성 수정본 충돌로 거부한다.
