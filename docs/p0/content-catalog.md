...
| [ADR-0033](../adr/0033-create-review-requests-with-user-owned-temporary-images.md#결정) | 사용자 소유 임시 대표 이미지 객체 검증과 생성 즉시 심사 요청 |
...
1. 승인된 운영자가 사용자 소유 임시 대표 이미지 객체와 업로드 서명 URL을 발급받아 이미지를 업로드한다.
2. 운영자가 대표 이미지 임시 객체 ID, 소개, 위치, 운영 시간, 연락처, 유의사항을 입력한다.
3. 날짜·회차별 정원, 연령·준비물, P0 취소 정책을 안내하는 문구와 공개 예정 시각을 입력한다.
4. 콘텐츠 생성 시 서버가 현재 인증 운영자를 `operator_id`로, 승인된 담당 지역을 `region_id`로 설정하고 임시 객체의 소유자·만료·S3 무결성을 검증해 연결한다.
5. 모든 필수 항목이 유효하면 콘텐츠를 생성 즉시 `PENDING`으로 만들고 심사를 요청한다.
6. 담당 지역 관리자가 콘텐츠와 공개 예정 시각을 함께 검토해 승인 또는 반려한다.
7. 승인된 콘텐츠는 `APPROVED` 상태로 유지되며 공개 예정 시각 전에는 일반 사용자에게 노출되지 않는다.
8. 승인된 공개 예정 시각이 되면 시스템이 콘텐츠를 한 번만 `PUBLISHED`로 전환하고 지역 홈과 목록에 노출한다.
9. 모든 회차가 `COMPLETED` 또는 `CANCELLED`가 된 뒤 종료일이 지나거나 정상 종료 처리되면 예약 접수와 노출을 종료한다.

#### 예외 흐름

- 필수 정보가 누락되면 승인 요청을 제출할 수 없다.
- 임시 객체가 다른 사용자의 것이거나 만료·미업로드·무결성 불일치면 콘텐츠를 만들지 않는다.
...
| [ADR-0033](../adr/0033-create-review-requests-with-user-owned-temporary-images.md#결정) | 임시 이미지 검증과 생성 즉시 심사 요청 |
| [P0 명세](../p0-spec.md#88-감사-및-운영-로그) | 최초 소유자 설정과 상태 전이 감사 요건 |

### 기능 범위

- 승인된 운영자는 사용자 소유 임시 대표 이미지 객체를 발급·업로드한 뒤 담당 지역에 콘텐츠를 생성하며 서버가 인증 운영자를 최초 소유자로 설정한다.
- 운영자는 콘텐츠 생성 요청에 표준 필수 정보, 회차별 일정·정원, 공개 예정 시각과 임시 이미지 객체 ID를 함께 등록한다.
- 신규 회차의 운영 상태는 `SCHEDULED`다.
- 필수 검증과 임시 이미지 검증을 통과한 콘텐츠는 생성 즉시 승인 요청 상태가 된다.

### 콘텐츠·회차 등록 정책

#### `CON-02`

승인된 운영자는 콘텐츠 생성 전에 사용자 소유 임시 대표 이미지 객체를 발급·업로드해야 한다. 콘텐츠 생성은 대표 이미지,
소개, 위치, 운영 시간, 연락처, 유의사항, 날짜·회차별 정원, 연령·준비물, P0 취소 정책을 안내하는 문구와 공개 예정
시각의 필수 검증 및 임시 객체의 소유자·만료·S3 무결성 검증을 통과해야 한다.
...
| [ADR-0033](../adr/0033-create-review-requests-with-user-owned-temporary-images.md#결정) | 임시 이미지 연결과 생성 즉시 수정 심사 요청 |
...
소유 운영자는 완전한 후보와 대표 이미지 스냅샷을 포함한 수정본을 생성 즉시 `EDIT_REQUESTED`로 요청하고,
...
`EDIT_REJECTED` 수정본은 소유 운영자가 보완한 뒤 `EDIT_REQUESTED`로 다시 요청할 수 있다. 대표 이미지를 바꾸는 경우에만
새 임시 대표 이미지 객체를 검증·연결하고, 바꾸지 않으면 기존 후보 이미지 스냅샷을 유지한다.
수정본 생성·심사 요청·승인·반려·철회의 상태, 처리자·사유·시각은 추가 전용 이력으로 보존하므로 재요청해도 기존 반려 사유가 사라지지 않는다.
...
철회 요청은 콘텐츠 행을 잠근 뒤 `PUBLISHED` 상태와 가장 최근 철회 관련 로그를 확인해 `content_log`에
`WITHDRAWAL_REQUESTED`·요청자·사유·시각을 추가 전용으로 기록하되 콘텐츠 상태는 `PUBLISHED`로 유지한다.
미종결 요청이 이미 있으면 새 로그를 추가하지 않고 기존 요청 결과를 반환한다.
담당 지역 관리자가 승인하면 같은 잠금 범위에서 `PUBLISHED → WITHDRAWN`으로 전환하고, 요청 사유를 보존한
`WITHDRAWN` `content_log`와 처리자·시각을 기록한다.
신규 홀드를 차단하고 `ACTIVE` 홀드를 무효화해 정원을 한 번 복구하지만,
기존 `CONFIRMED` 예약은 명시적인 회차 취소가 없으면 유지한다.
...
운영 중단·전체 콘텐츠 `WITHDRAWAL_REQUESTED`·`WITHDRAWN`·종료·삭제의 상태 전이는 감사 대상이다.
...
| 임시 대표 이미지 객체 | `image_object_id`, `temporary_owner_user_id`, `temporary_expires_at`, lifecycle, object key, 검증 메타데이터 | 생성 전 업로드 소유권·만료·무결성 검증과 단일 연결 |
