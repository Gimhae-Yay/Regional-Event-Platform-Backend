...
9. 모든 회차가 `COMPLETED` 또는 `CANCELLED`가 되면 시스템이 콘텐츠를 한 번만 `ENDED`로 전환해 예약 접수와 노출을 종료한다. 지역 관리자는 같은 조건에서 스케줄러 실행을 기다리지 않고 정상 종료 처리할 수 있다.
...
- 모든 회차가 `COMPLETED` 또는 `CANCELLED`가 되면 시스템이 콘텐츠를 한 번만 자동 종료한다.
