...
3. 시스템이 공통 인증·인가 계약과 서명, 만료 시각, 체크인 창과 회차 상태를 검증한 뒤 예약 상태와 기존 방문을 함께 확인한다.
...
- 새로운 스캔은 공통 인증·인가 계약과 QR·시간·회차 상태를 다시 검증하고, 검증을 통과했더라도 이미 `CHECKED_IN`된
...
새로운 QR 스캔은 Access Token의 `ROLE_OPERATOR` authority를 1차로 확인한 뒤, 활성 `ORDINARY` 계정·현재 담당 지역·콘텐츠 소유 관계와 서명, 만료 시각, 체크인 창,
`SCHEDULED` 회차, 행사·회차 일치, 예약의 활성 회원 연결을 검증하고
...
처리 전 확인 사유, 처리자와 처리 시각을 기록하고 공통 인증·인가 계약, 체크인 창·회차 상태를 모두 검증한 뒤 체크인한다.
