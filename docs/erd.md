...
| 테이블 | 역할 |
| --- | --- |
| `audit_event` | 지역·콘텐츠·회차·홀드·예약 등의 성공·실패 상태 전이와 사유를 90일간 기록한다. 개인정보와 QR 원문은 저장하지 않는다. |
...
- `reservation_no`는 시스템 전체에서 유일한 예약 번호다. QR 실패 시 운영자가 예약 번호만으로
  정확히 한 예약을 보조 조회할 수 있도록 `UNIQUE` 제약을 둔다. 서버는 `RyyyyMMdd` 뒤에
  12자리 Crockford Base32 난수 접미사를 붙여 생성한다. 날짜는 `Asia/Seoul` 기준이며,
  이름·연락처·`user_id`를 포함하지 않는다. 유일 제약 충돌 시 새 접미사로 재시도한다.
- `qr_reference`는 QR에 넣는 불투명 예약 참조다. 서버가 UUID v4 표준 문자열로 생성하며,
  이름·연락처·`user_id`를 QR에 넣지 않는다. 유일 제약 충돌 시 새 UUID v4를 생성한다.
...
- `SUCCEEDED`와 저장 대상 `FAILED` 기록은 `completed_at`부터 24시간 보관한다. `expires_at`은
  완료 시각에 24시간을 더해 갱신하고, MySQL 현재 시각이 이를 지난 종결 기록만 정리 작업에서 제거한다.
- 동일 키 경합의 DB 대기 제한은 운영 설정으로 짧게 관리한다. 제한을 넘긴 요청은
  `IDEMPOTENCY_REQUEST_IN_PROGRESS`로 응답하고 새 도메인 작업을 실행하지 않는다.
  `PROCESSING` 기록을 만료·정리·재점유해 자동 탈취하지 않으며, 비정상적으로 오래 유지되는 기록은
  운영 경보와 수동 조사 대상으로 남긴다.
...
- 예약 확정 성공은 같은 `request_id`로 두 이벤트를 기록한다. 하나는
  `target_type = CAPACITY_HOLD`, 대상 홀드, `ACTIVE → CONSUMED`이고 다른 하나는
  `target_type = RESERVATION`, 생성 예약, `NULL → CONFIRMED`다. 두 이벤트와 actor link는
  홀드 소비·예약 생성·멱등 성공 기록과 같은 트랜잭션에서 커밋한다.
