...
- `빌드 및 테스트` 작업 전체는 3분 49초, 같은 이름의 Gradle 단계는 3분 23초가 걸렸다.
...
| GitHub Actions | 측정 전 | 작업 3분 49초, Gradle 단계 3분 23초 | 기준 없음 | 성공 여부와 실제값 기록 |
...
- GitHub Actions의 실제 After 시간은 기록했지만 동일 runner 조건의 Before 값이 없어 CI 성능 개선으로 비교하지 않는다.
...
- 관련 이슈·PR: GitHub #322, 기준 PR #319, 구현 PR #328,
  GitHub Actions run `30789214176`
