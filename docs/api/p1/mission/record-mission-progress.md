...
| 실패 재처리 | 자동 재시도 없이 동일 `visitId`로 내부 유스케이스를 명시적으로 재호출 |
| `requestId` | HTTP 흐름은 원 요청 값을 전달하고, 비-HTTP 호출자는 실행 경계에서 한 번 생성해 전체 후보 처리에 유지 |
...
10. 조건 불충족과 이미 반영된 방문은 정상 무변경으로 처리하며 실패 로그를 만들지 않는다. 대상 참여 처리 중
    예상하지 못한 시스템 실패는 `ERROR`로 기록하고 `requestId`, `visitId`, `missionId`,
    `missionParticipationId`와 비개인 오류 코드만 포함한다. 후보 참여를 식별하기 전에 실패하면 확인 가능한
    `requestId`, `visitId`와 비개인 오류 코드만 포함한다. 토큰, 사용자 개인정보, 방문 상세, 예외 메시지와
    스택 트레이스는 기록하지 않는다.
