...
| `403` | `FORBIDDEN` | Access Token에 `ROLE_VISITOR` authority가 없거나 활성 `ORDINARY` 계정이 아니거나 본인 참여가 아니다. 수령 이력과 쿠폰을 만들지 않는다. |
...
1. Access Token의 `ROLE_VISITOR` authority를 1차로 확인하고, DB에서 활성 `ORDINARY` 계정·경로 식별자·참여 존재 여부와 본인 소유를 확인한 뒤 기존 보상 수령 이력을 먼저 조회한다.
