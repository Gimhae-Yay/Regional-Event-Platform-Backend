...
이 문서는 활성 `SUPER_ADMIN` 또는 `PLATFORM_ADMIN`이 `ORDINARY` 계정에 지역 관리자(`REGION_ADMIN`) 역할과
...
활성 `SUPER_ADMIN` 또는 `PLATFORM_ADMIN`이 `user_role_assignment`에 `REGION_ADMIN` 배정을 새로 만들거나
...
| `403` | `FORBIDDEN` | 인증 주체가 활성 `SUPER_ADMIN` 또는 `PLATFORM_ADMIN` 고권한 배정을 갖지 않는다. 역할·감사 이벤트를 변경하지 않는다. |
