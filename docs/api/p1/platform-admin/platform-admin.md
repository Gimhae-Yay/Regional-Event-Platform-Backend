...
이 문서는 전체관리자의 HTTP API 계약을 관리한다. 역할 보호 경로는 `ROLE_SUPER_ADMIN` 또는
`ROLE_PLATFORM_ADMIN` authority snapshot으로 1차 인가하며, DB에서는 활성 `PRIVILEGED` 계정과 대상·업무 상태를
검증한다. 사용자 목록은 활성 일반 계정과 일반 역할만 반환한다. 고권한 계정 생성·비활성화는
`ROLE_SUPER_ADMIN`만 수행할 수 있고, 지역 관리자(`REGION_ADMIN`) 역할과 담당 지역의 임명·회수는
`ROLE_SUPER_ADMIN` 또는 `ROLE_PLATFORM_ADMIN`이 수행한다.
...
| 인증·인가 | [인증·인가](../../common/authentication.md) | 목록 조회와 지역관리자 역할 변경은 `ROLE_SUPER_ADMIN` 또는 `ROLE_PLATFORM_ADMIN`, 고권한 계정 변경은 `ROLE_SUPER_ADMIN` snapshot이 필요하다. 모든 역할 보호 API는 DB에서 활성 `PRIVILEGED` 계정과 대상·업무 상태를 추가로 검증한다. |
