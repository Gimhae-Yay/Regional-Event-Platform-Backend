...
| 소유 도메인 | 전체관리자 |
| 기준 문서 | [전체관리자 API](platform-admin.md), [전체관리자](../../../p1/platform-admin.md), [P1 ERD](../../../p1-erd.md), [ADR-0086](../../../adr/0086-apply-separated-privileged-account-model-to-platform-admin-api.md), [API 공통 계약](../../common/README.md) |
...
| `data.users` | Array | 활성 일반 계정 배열이다. 결과가 없으면 빈 배열이다. |
...
| `data.users[].roleAssignments` | Array | 현재 `ACTIVE` 상태인 역할·담당 지역 연결이다. 역할이 없으면 빈 배열이다. |
| `data.users[].roleAssignments[].role` | String | `VISITOR`, `OPERATOR`, `REGION_ADMIN` 중 하나다. |
| `data.users[].roleAssignments[].regionId` | String 또는 null | `VISITOR`이면 `null`이고, `OPERATOR`, `REGION_ADMIN`이면 담당 지역 식별자다. |
