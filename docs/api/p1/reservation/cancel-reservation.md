...
7. 가격 스냅샷의 `final_amount = 0`이고 결제 행이 없으면 환불을 만들지 않는다. 쿠폰이 적용됐다면 예약의 `hold_id`와 가격 스냅샷의 `hold_id`, 사용 이력의 `reservation_id`·`price_snapshot_id`·`coupon_id`가 각각 취소 대상 예약·스냅샷·적용 쿠폰과 일치하고, 사용 이력은 `CONFIRMED`, 쿠폰은 `USED`인지 검증한다. 최초 예약 취소 트랜잭션에서 검증된 `coupon_redemption`을 `REVERSED`로 전이하고, 원래 만료 시각 전이면 쿠폰을 `AVAILABLE`, 지났으면 `EXPIRED`로 전이해 상태 이력을 기록한다. 적용 쿠폰이 없으면 쿠폰 처리 없이 `refund: null`로 취소한다.
