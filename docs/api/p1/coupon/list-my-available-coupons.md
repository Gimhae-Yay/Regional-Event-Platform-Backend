...
활성 회원이 특정 활성 홀드와 가격 스냅샷에 사용할 수 있는 본인 쿠폰 목록을 조회한다.
...
```http
GET /api/v1/me/coupons/available?holdId=789&priceSnapshotId=9001 HTTP/1.1
...
| `priceSnapshotId` | String | Y | 쿠폰 적용 금액을 판단할 가격 스냅샷 식별자. 양수여야 한다. |
...
| `data.priceSnapshotId` | String | 사용 가능 판단 대상 가격 스냅샷 식별자 |
...
| `data.availableCoupons[].discountPreview.baseAmount` | Number | 쿠폰 적용 전 가격 스냅샷 금액 |
...
| --- | --- | --- |
| `400` | `INVALID_INPUT` | `holdId` 또는 `priceSnapshotId`가 양수가 아니다. 조회 대상과 상태를 변경하지 않는다. |
| `400` | `INVALID_TYPE` | `holdId` 또는 `priceSnapshotId`의 형식이 올바르지 않다. |
| `401` | `UNAUTHENTICATED` | 인증 정보가 없거나 유효하지 않다. |
| `403` | `FORBIDDEN` | 인증 주체가 활성 회원이 아니거나 홀드 또는 가격 스냅샷의 소유자가 아니다. |
| `404` | `NOT_FOUND` | 대상 홀드 또는 가격 스냅샷을 찾을 수 없다. |
| `409` | `COUPON_AVAILABILITY_CONFLICT` | 홀드와 가격 스냅샷의 연결이 일치하지 않거나 쿠폰 적용 판단이 불가능한 상태다. |
...
2. 홀드는 만료 시각 전의 `ACTIVE` 상태여야 한다. 홀드와 가격 스냅샷은 인증 회원의 소유여야 하며 가격 스냅샷은 대상 홀드에 연결돼 있어야 한다.
3. `AVAILABLE` 상태이고 `expiresAt > evaluatedAt`인 쿠폰만 반환한다.
4. 쿠폰 정책의 `contentId`는 홀드 회차의 콘텐츠와 같아야 하며 `regionId`도 일치해야 한다.
5. 가격 스냅샷 금액이 `minimumPaymentAmount` 미만인 쿠폰은 반환하지 않는다.
6. 정책이 종료됐더라도 이미 발급된 쿠폰은 자체 만료 시각과 사용 조건을 기준으로 판단한다.
7. 이 API는 조회 전용이며 쿠폰 예약, 사용 확정, 상태 이력 생성을 수행하지 않는다.
