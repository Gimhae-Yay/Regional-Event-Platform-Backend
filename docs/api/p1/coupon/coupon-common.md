...
| `RESERVED` | 만료 전에 결제 시작 또는 가격 스냅샷에 선점된 쿠폰 상태. 원래 만료 시각을 연장하지 않지만 연결된 홀드가 `ACTIVE`인 동안 선점 효력을 유지한다. |
...
| `AVAILABLE → RESERVED` | 결제 시작 또는 0원 예약의 가격 확정 처리가 가격 스냅샷에 쿠폰과 할인 금액을 고정하면서 선점한다. |
| `RESERVED → USED` | 쿠폰 사용 확정이 같은 가격 스냅샷의 유효한 `ACTIVE` 홀드를 소비해 `CONFIRMED` 예약을 생성하고 `coupon_redemption(CONFIRMED)`을 같은 트랜잭션에서 처리한다. 선점 뒤 원래 만료 시각이 지나도 활성 홀드 안에서는 확정할 수 있다. |
| `RESERVED → AVAILABLE` | 결제·홀드가 예약 확정 전에 종료되고 원래 만료 시각 전이면 선점을 해제한다. |
| `AVAILABLE → EXPIRED` | 만료 작업이 원래 만료 시각이 지난 미사용 쿠폰을 전이하고 상태 이력을 남긴다. |
| `RESERVED → EXPIRED` | 결제·홀드가 예약 확정 전에 종료되고 원래 만료 시각이 지났으면 선점 해제 처리가 전이하고 상태 이력을 남긴다. |
...
`AVAILABLE → USED` 직접 전이는 허용하지 않는다. 모든 전이는 원인, 처리자 또는 시스템 주체와 처리 시각을 상태 이력에 기록한다.
