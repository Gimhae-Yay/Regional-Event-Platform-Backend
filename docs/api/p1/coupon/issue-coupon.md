...
활성 회원이 공개된 쿠폰 정책과 본인 발급 근거를 기준으로 쿠폰 발급을 요청한다.
방문은 정책·수령자, 미션·스탬프북은 정책·수령자·보상 근거가 같은 반복 요청에서 기존 쿠폰을 반환한다.
...
| Name | Type | Required | Description |
| --- | --- | --- | --- |
| `issueSourceType` | String | Y | `VISIT`, `MISSION_REWARD`, `STAMPBOOK_COMPLETION` 중 하나. 정책의 발급 경로와 같아야 한다. |
| `sourceId` | String | Y | 발급 근거 식별자. 방문, 미션 보상 수령 또는 스탬프북 완료 보상 식별자 |
...
| --- | --- | --- |
| `400` | `INVALID_INPUT` | `couponPolicyId`, `issueSourceType` 또는 `sourceId`가 없거나 형식·범위가 올바르지 않다. 쿠폰은 발급하지 않는다. |
...
2. 요청의 `issueSourceType`은 쿠폰 정책의 발급 경로와 같아야 한다.
3. 발급 근거가 존재하고 인증 회원의 소유인지 검증한다.
4. 방문 발급 식별 키는 정책·수령자로 계산하며 `sourceId`는 자격 증거로만 보존한다. 미션·스탬프북 발급 식별 키는 정책·수령자·보상 근거로 계산한다.
...
7. 신규 발급의 근거는 인증 회원의 유효 방문, 미션 보상 수령 또는 스탬프북 완료 보상 중 하나여야 한다.
8. 신규 발급에서 미션 보상 수령과 스탬프북 완료 보상 근거는 원본 보상 정책이 요청 정책과 일치하고 정책 콘텐츠와 같은 지역이어야 한다.
9. 신규 발급에서 방문 근거는 정책의 `contentId`와 같은 콘텐츠에서 생성된 유효 방문이어야 한다.
10. 신규 발급 시 쿠폰 상태는 `AVAILABLE`이며 `expiresAt = issuedAt + validDaysAfterIssue`로 계산한다.
