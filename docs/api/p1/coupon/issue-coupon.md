...
스탬프북 완료 보상은 본인 `COMPLETED` 진행의 [내 스탬프북 목록](../stampbook/list-my-stampbooks.md) 또는
[내 스탬프북 상세·진행도](../stampbook/get-my-stampbook.md) 응답에 함께 반환된 `completionReward`로 요청을 구성한다.
...
| --- | --- | --- | --- |
| `couponPolicyId` | String | Y | 발급 근거가 되는 쿠폰 정책 식별자다. 양수여야 한다. `STAMPBOOK_COMPLETION`이면 본인 `COMPLETED` 진행의 `completionReward.couponPolicyId`를 사용한다. |
...
| `sourceId` | String | Y | 발급 근거 식별자다. `VISIT`이면 방문 식별자이고, `STAMPBOOK_COMPLETION`이면 본인 `COMPLETED` 진행의 `completionReward.stampbookRewardGrantId`다. |

#### Request Example (`STAMPBOOK_COMPLETION`)
...
{
  "issueSourceType": "STAMPBOOK_COMPLETION",
  "sourceId": "9001"
}
```
...
4. `STAMPBOOK_COMPLETION` 요청은 본인 `COMPLETED` 진행의 `completionReward.couponPolicyId`를 경로 `couponPolicyId`로, `completionReward.stampbookRewardGrantId`를 본문 `sourceId`로 사용한다. `COMPLETED`가 아닌 진행은 `completionReward = null`이므로 이 발급 경로를 구성할 수 없다.
5. 방문 발급 식별 키는 정책·수령자로 계산하며 `sourceId`는 자격 증거로만 보존한다. 스탬프북 발급 식별 키는 정책·수령자·보상 근거로 계산한다.
6. 같은 발급 식별 키의 쿠폰이 이미 있으면 현재 정책 상태·발급 기간·전체 발급 한도와 근거의 현재 유효 상태를 다시 검증하지 않고 기존 쿠폰을 반환한다. 쿠폰과 발급 이력은 새로 만들지 않는다.
7. 기존 발급 결과가 없는 경우에만 쿠폰 정책이 `PUBLISHED` 상태이고 서버 현재 시각이 `issueStartsAt` 이상, `issueEndsAt` 이하인지 검증한다.
8. 신규 발급의 근거는 인증 회원의 유효 방문 또는 스탬프북 완료 보상이어야 한다.
9. 신규 발급에서 스탬프북 완료 보상 근거는 원본 보상 정책이 요청 정책과 일치하고 정책 콘텐츠와 같은 지역이어야 한다.
10. 신규 발급에서 방문 근거는 정책의 `contentId`와 같은 콘텐츠에서 생성된 유효 방문이어야 한다.
11. 신규 발급 시 `validDaysAfterIssue`는 1 이상 365 이하이고, 쿠폰 상태는 `AVAILABLE`이며 `expiresAt = issuedAt + validDaysAfterIssue`로 계산한다.
12. 신규 발급 트랜잭션은 정책 잠금 또는 동등한 직렬화 수단을 확보한 뒤 같은 발급 식별 키를 다시 조회한다. 기존 쿠폰이 확인되면 발급 한도 사용량을 증가시키지 않고 `duplicate = true`인 기존 결과를 반환한다.
13. 재조회 후에도 기존 결과가 없고 전체 발급 한도가 있으면 잔여 한도를 확인하고 사용량 증가와 쿠폰 생성을 원자적으로 처리한다.
14. 동시 요청으로 발급 식별 키 유일 제약 충돌이 발생하면 시도한 신규 발급 트랜잭션을 롤백하고 기존 쿠폰을 조회해 `duplicate = true`로 반환한다. 알려진 발급 식별 키 충돌을 `500 INTERNAL_SERVER_ERROR`로 노출하거나 발급 한도 사용량을 추가로 증가시키지 않는다.
15. 쿠폰, 발급 이력, 상태 이력과 발급 한도 사용량은 하나의 트랜잭션에서 함께 커밋한다.
16. 발급 성공과 실패 사유는 구조화 로그와 감사 이력으로 추적한다.
