...
{
  "isPublic": false,
  "reasonCode": "REGION_OPERATION_ENDED",
...
| `reasonCode` | String | Y | 비개인 공개 여부 변경 사유 코드. 앞뒤 공백 제거 후 목표 `isPublic`에 맞는 아래 허용 코드 중 하나여야 한다. 실제 상태 전이에 성공하면 `audit_event.reason_code`에 저장하고 동일 상태 무변경 성공에서는 저장하지 않는다. |
| `evidenceReference` | String | Y | 비밀값과 개인정보를 포함하지 않는 내부 증빙 참조. 앞뒤 공백 제거 후 1자 이상 500자 이하여야 한다. 실제 상태 전이에 성공하면 `audit_event.evidence_reference`에 저장하고 동일 상태 무변경 성공에서는 저장하지 않는다. |
...
| `isPublic = false` | `REGION_OPERATION_ENDED` | 지역 운영 종료 |
...
| `409` | `REGION_AVAILABILITY_CONFLICT` | 비삭제 콘텐츠가 있는 공개 지역을 비공개로 바꾸려 한다. 지역과 성공 감사 이력은 변경하지 않고 롤백 뒤 실패 감사 이벤트만 별도 트랜잭션으로 기록한다. 비삭제 콘텐츠가 모두 제거된 뒤 재시도할 수 있다. |
...
6. `true → false`는 `content.region_id = regionId`이고 `content.deleted_at IS NULL`인 콘텐츠가 하나도 없을 때만 허용한다.
7. 6번 조건을 만족하지 않으면 `409 REGION_AVAILABILITY_CONFLICT`를 반환하고 성공 감사 이벤트를 만들지 않는다. 롤백 완료 뒤 같은 오류 코드를 `reason_code`로 가진 실패 감사 이벤트를 별도 트랜잭션으로 기록한다.
...
9. 성공 감사 이벤트에는 토큰, 비밀값과 개인정보를 저장하지 않는다. 활성 처리자는 `audit_event_actor_link`에 연결한다.
