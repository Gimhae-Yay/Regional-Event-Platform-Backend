...
| `data.regions[].regionAdminCount` | Number | 활성 지역 관리자 배정 수 |
...
| --- | --- | --- |
| `400` | `INVALID_INPUT` | `isPublic`이 허용 값이 아니다. 조회 대상과 상태를 변경하지 않는다. |
| `401` | `UNAUTHENTICATED` | Access Token이 없거나 유효하지 않다. 조회 대상과 상태를 변경하지 않는다. |
| `403` | `FORBIDDEN` | 인증 주체가 활성 `PLATFORM_ADMIN`이 아니다. 조회 대상과 상태를 변경하지 않는다. |
| `500` | `INTERNAL_SERVER_ERROR` | 조회 중 예상하지 못한 서버 오류가 발생했다. 조회 대상과 상태를 변경하지 않는다. |
...
5. `regionAdminCount`는 활성 `REGION_ADMIN` 역할 배정만 집계한다.
