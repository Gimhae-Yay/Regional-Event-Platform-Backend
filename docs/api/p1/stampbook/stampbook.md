...
이 문서는 운영자의 스탬프북 목록·상세 조회·생성·수정·공개 심사 요청·종료, 지역 관리자의 심사 대기 조회·승인·반려와
...
| 페이지네이션 | [페이지네이션](../../common/pagination.md) | 운영자·내 스탬프북 목록·적립 이력과 지역 관리자 심사 대기 목록은 단순 목록이고, 나머지는 단건·명령 API이므로 페이지네이션을 적용하지 않는다. |
...
| 운영자 스탬프북 목록 조회 | `GET /operator/stampbooks` | [list-operator-stampbooks.md](list-operator-stampbooks.md) |
| 운영자 스탬프북 상세 조회 | `GET /operator/stampbooks/{stampbookId}` | [get-operator-stampbook.md](get-operator-stampbook.md) |
