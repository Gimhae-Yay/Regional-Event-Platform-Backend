...
이 문서는 유료 예약 결제의 생성·서버 승인 확인·상태 조회, PortOne 웹훅 수신, 담당 예약 결제 현황 조회와
전체관리자의 결제 불일치 조회·수동 조치 HTTP 계약을 정의한다.

**환불은 이 문서의 범위가 아니다.** 취소·환불 요청, 내 환불 상태 조회, 환불 재시도와 결제 불일치의 전액환불
요청 처리는 별도 환불 도메인 문서가 소유한다. 이 문서의 "결제 불일치 문제없음 종결"은 전체관리자가 조사
결과 조치가 필요 없다고 판단한 경우에만 사용하며, 전액환불이 필요하면 환불 도메인의 API로 이관한다.
...
| P1-FR-07, PAY-02, PAY-03 | `POST /api/v1/me/payments/{paymentId}/confirm` | `payment`, `payment_verification`, `reservation`, `coupon_redemption`, `payment_discrepancy` |
| P1-FR-07, PAY-06 | `GET /api/v1/me/payments/{paymentId}` | `payment` |
| P1-FR-07, PAY-06 | `GET /api/v1/operator/payment-reconciliations` | `payment`, `reservation`, `payment_discrepancy` |
| P1-FR-07, PAY-04 | `POST /api/v1/webhooks/portone` | `payment_webhook`, `payment` |
...
| 멱등성 | [ADR-0069](../../../adr/0069-use-p0-capacity-hold-and-reservation-price-snapshot-for-paid-checkout.md) | 결제 생성은 `Idempotency-Key`와 전용 `payment_idempotency`로 24시간 보관하는 영속 멱등 처리 대상이다. 서버 승인 확인과 웹훅 수신은 `payment.status` 및 `payment_webhook.provider_event_id` 유일성으로 자연 멱등을 보장하며 별도 `Idempotency-Key`를 받지 않는다. |
...
| 서버 결제 승인 확인 | `POST /me/payments/{paymentId}/confirm` | [confirm-payment.md](confirm-payment.md) |
| 내 결제 상태 조회 | `GET /me/payments/{paymentId}` | [get-my-payment.md](get-my-payment.md) |
| 담당 예약 결제 상태 조회 | `GET /operator/payment-reconciliations` | [list-operator-payment-reconciliations.md](list-operator-payment-reconciliations.md) |
| PortOne 결제 웹훅 수신 | `POST /webhooks/portone` | [receive-portone-webhook.md](receive-portone-webhook.md) |
