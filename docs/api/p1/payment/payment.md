...
| 인증·인가 | [인증·인가](../../common/authentication.md) | 방문자 API(`/me/...`)는 활성 회원 본인 소유 조건을 검증한다. 운영자 API(`/operator/...`)는 `ROLE_OPERATOR` snapshot과 활성 `ORDINARY` 계정·현재 담당 지역·콘텐츠·예약 소유권을, 전체관리자 API(`/platform-admin/...`)는 `ROLE_SUPER_ADMIN` 또는 `ROLE_PLATFORM_ADMIN` snapshot과 활성 `PRIVILEGED` 계정을 검증한다. PortOne 웹훅은 `Authorization` Bearer 인증 대신 공급자 서명 검증을 사용하며 [인증 제외 API](../../common/authentication.md) 표에 추가한다. |
