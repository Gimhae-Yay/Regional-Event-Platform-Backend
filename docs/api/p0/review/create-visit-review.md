...
| `reviewText` | String | Y | 앞뒤 공백을 제거한 값이 `1~1,000`자여야 한다. `null`, 빈 문자열, 공백만으로 된 값은 허용하지 않는다. |
...
| --- | --- | --- |
| 400 | `INVALID_TYPE` | `visitId`를 양의 10진 정수 문자열로 처리할 수 없다. 상태를 변경하지 않는다. |
...
- 회원 탈퇴 또는 동시 작성과 경합하면 먼저 조건을 만족해 커밋한 처리만 적용한다. 거부·실패는 후기 및 성공 감사 이벤트와 별도 트랜잭션에서 서버가 확인한 `requestId`, 행위자 역할·식별값, 대상 지역, 이전·이후 상태와 결과 코드를 남긴다. 탈퇴하거나 연결이 제거된 작성자에는 행위자 연결을 만들지 않는다.
