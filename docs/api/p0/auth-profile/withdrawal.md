...
Redis 폐기 성공 뒤에는 하나의 MySQL 트랜잭션에서 `ACTIVE` 홀드를 `INVALIDATED`로 전환하고, 최초 성공 전이에서만
...
제거하고, 요청 지역·비개인 상태·처리 사유·시각만 유지한다. 회원·프로필·자격 증명·직접 식별 정보를 파기하고, 방문 기록의
회원 연결을 제거한다. 기존 공개 후기는 작성자 표시를 공통 `탈퇴한 사용자`로 바꾸되 콘텐츠·회차·평점·본문·상태는 보존한다.
