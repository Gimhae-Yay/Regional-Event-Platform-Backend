...
현재 회원이 실제로 부여받은 역할과 담당 지역을 조회한다. `VISITOR` 역할의 `regionId`는 항상 `null`이며,
`OPERATOR`와 `REGION_ADMIN` 역할의 `regionId`는 항상 양의 정수이다.
...
| `data.userId` | Long | 인증된 회원의 식별자. 양의 정수이다. |
