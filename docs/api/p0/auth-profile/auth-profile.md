...
HTTP API 계약을 관리한다. 공개 API는 회원가입·로그인·토큰 갱신·로그아웃이고, 회원탈퇴·내 역할 조회·본인 운영자
신청 현황 조회·운영자 신청과 지역 관리자 심사 API는 Access Token을 요구한다. 요청·응답 공통 형식과 인증 전달 방식은 `common/` 문서를 단일
...
| 인증·인가 | [인증·인가](../../common/authentication.md) | 회원가입·로그인·토큰 갱신·로그아웃은 인증 제외 API다. 회원탈퇴·내 역할 조회·본인 운영자 신청 현황 조회·운영자 신청에는 Access Token과 활성 회원 상태가 필요하며, 지역 관리자 심사 API에는 추가로 `REGION_ADMIN` 역할과 담당 지역 일치가 필요하다. |
...
| 본인 운영자 신청 현황 조회 | `GET /api/v1/me/operator-application` | [my-operator-application.md](my-operator-application.md) |
