...
이 문서는 인증·프로필 도메인의 회원가입, 로그인, 토큰 갱신, 로그아웃과 회원탈퇴 HTTP API 계약을 관리한다.
...
| 인증·인가 | [인증·인가](../../common/authentication.md) | 회원가입·로그인·토큰 갱신·로그아웃은 인증 제외 API이며, 회원탈퇴에는 Access Token이 필요하다. |
