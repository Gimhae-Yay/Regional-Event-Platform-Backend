...
- 소유 운영자가 `canCheckIn = true`를 확인했으면 새 `checkInRequestId`를 생성하고, 조회에 사용한
  `reservationNo`와 현장 확인 사유를 [예약번호 보조 조회 후 체크인](../check-in/manual-check-in-by-reservation-number.md)에
  전달해 실제 체크인을 요청한다.
- `canCheckIn`은 조회 시점의 안내 값이며 체크인 권한을 증명하는 토큰이 아니다. 후속 명령은 운영자 소유·지역,
  예약·회차·회원 상태와 체크인 창을 MySQL 현재 상태로 다시 검증한다.
- 지역 관리자는 이 후속 체크인 명령을 호출할 수 없다.
