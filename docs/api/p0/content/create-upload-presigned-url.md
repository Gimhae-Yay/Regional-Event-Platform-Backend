...

{
  ...
  "byteSize": 245761,
  "checksum": "6b1f8b5f7b963d9c9f4a4d883b53f8ff"
}
```
...
| `checksum` | String | Y | 공백만으로 구성될 수 없는 업로드 무결성 검증값 |
...
```json
{
  ...
  "data": {
    ...
    "uploadUrlExpiresAt": "2026-07-30T14:20:00+09:00",
    "temporaryObjectExpiresAt": "2026-07-30T15:00:00+09:00"
  }
}
```
...
| `data.imageObjectId` | Number | 서버가 생성하고 인증된 운영자에게 소유시킨 `TEMPORARY` 이미지 객체 식별자 |
...
1. 서버는 인증된 승인 운영자의 소유로 `TEMPORARY` 이미지 객체와 서버 생성 객체 키를 만들고, 미디어 타입·바이트 크기·체크섬을 저장한다. 콘텐츠·수정본 식별자는 요청받거나 연결하지 않는다.
2. 클라이언트는 `uploadUrl`로 비공개 S3에 직접 업로드한다. 이 API는 파일 바이트를 받지 않고 업로드 완료를 보장하지 않는다.
3. 클라이언트는 임시 객체 만료 전에 콘텐츠 생성·콘텐츠 보완·수정본 생성·수정본 보완 요청에서 `imageObjectId`를 지정할 수 있다. 수정본 재요청은 이미지 객체를 받지 않으며, 이미지 변경이 없는 경우 새 임시 객체 발급·검증을 요구하지 않는다. 연결 시 서버는 업로드 검증과 소유자를 확인하고 임시 만료를 적용하지 않는다.
