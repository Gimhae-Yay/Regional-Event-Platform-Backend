...
| `AUTH-01` | `GET /operator/contents` | `ROLE_OPERATOR` snapshot, 활성 `ORDINARY` 계정, `content.operator_id`, `content.region_id` |
...
| 인증·인가 | [인증·인가](../../common/authentication.md) | `ROLE_OPERATOR` snapshot으로 1차 인가하고, DB에서 활성 `ORDINARY` 계정과 본인 소유 콘텐츠 조건을 확인한다. |
...
| `403` | `FORBIDDEN` | `ROLE_OPERATOR` authority가 없거나 활성 `ORDINARY` 계정 또는 담당 지역 관계가 없다. 콘텐츠를 반환하지 않는다. |
