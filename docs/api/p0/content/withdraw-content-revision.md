...
| `409` | `CONTENT_STATE_CONFLICT` | 수정본이 `EDIT_REQUESTED`도 기존 철회 결과도 아니며, 콘텐츠 중단·종료로 `EDIT_INVALIDATED`가 된 경우를 포함한다. 수정본을 변경하지 않는다. |
...
1. `EDIT_REQUESTED` 상태에서만 최초 철회가 성공한다. 승인·반려·철회·콘텐츠 중단·종료가 경합하면 조건부 상태 전이로 최초 하나만 성공한다. 콘텐츠 중단·종료가 먼저 `EDIT_INVALIDATED`를 커밋하면 이 요청은 `409 CONTENT_STATE_CONFLICT`로 거부한다.
