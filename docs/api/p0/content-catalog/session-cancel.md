...
| Name | Type | Required | Description |
| --- | --- | --- | --- |
| `cancellationReason` | String | Y | 앞뒤 공백을 제거한 비어 있지 않은 텍스트여야 한다. `null`, 빈 문자열, 공백만으로 된 값은 허용하지 않는다. 최대 길이는 별도로 제한하지 않는다. |
...
| --- | --- | --- |
| 400 | `INVALID_INPUT` | 회차 식별자가 양의 정수가 아니거나 취소 사유가 누락·공백이다. 회차·홀드·예약은 변경되지 않으며 값을 수정해 다시 요청할 수 있다. |
