...
감사 기록을 하나의 트랜잭션으로 함께 커밋한다. 요청 본문으로 공개 예정 시각을 변경하지 않는다.
...
| 409 | `CONTENT_STATE_CONFLICT` | 콘텐츠가 `PENDING`이 아니거나 동시 승인·반려 요청이 먼저 처리됐다. 상태와 이력은 변경하지 않으며 최신 상세를 조회한 뒤 판단한다. |
