...
공개 전 수정 심사로 원본이 `PENDING`인 수정본을 함께 반환한다. 콘텐츠 중단·전체 철회·종료로 `EDIT_INVALIDATED`가 된
...
2. 수정본은 `EDIT_REQUESTED`, 원본은 소프트 삭제되지 않은 상태여야 한다. 콘텐츠가 `SUSPENDED`, `WITHDRAWN` 또는 `ENDED`로
...
- 콘텐츠 중단·전체 철회·종료와 경합해 수정본이 `EDIT_INVALIDATED`로 커밋되면 해당 수정본은 목록에서 제외한다. 이미 종결된
