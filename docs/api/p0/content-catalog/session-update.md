...
| `sessionId` | String | Y | API 공통 규칙을 따르는 회차 식별자다. |
...
| `data.sessionId` | String | API 공통 규칙에 따른 수정한 회차 식별자 |
| `data.contentId` | String | API 공통 규칙에 따른 회차의 콘텐츠 식별자 |
| `data.status` | String | 수정 후에도 유지되는 `SCHEDULED` |
| `data.startsAt` | String | 수정된 회차 시작 일정 시각. API 공통 규칙에 따른 `Asia/Seoul` ISO 8601 오프셋 일시다. |
| `data.endsAt` | String | 수정된 회차 종료 일정 시각. API 공통 규칙에 따른 `Asia/Seoul` ISO 8601 오프셋 일시다. |
| `data.checkinOpenAt` | String | 수정된 체크인 시작 일정 시각. API 공통 규칙에 따른 `Asia/Seoul` ISO 8601 오프셋 일시다. |
| `data.checkinCloseAt` | String | 수정된 체크인 종료 일정 시각. API 공통 규칙에 따른 `Asia/Seoul` ISO 8601 오프셋 일시다. |
