...
| --- | --- | --- |
| Base URL·미디어 타입·시간 형식 | [API 공통 규칙](../../common/api-conventions.md) | 실제 경로는 `/api/v1/region-admin/content-revisions/{revisionId}`; 생성·수정·심사·처리 이벤트 시각은 ISO 8601 UTC `Z` 문자열이며 콘텐츠 일정값만 `+09:00` 오프셋 문자열 |
...
| --- | --- | --- | --- |
| `revisionId` | String | Y | 조회할 수정본 식별자. 양의 10진 문자열이어야 한다. |
...
```json
{
  ...
  "data": {
    ...
    "candidateImageUrlExpiresAt": "2026-07-30T00:30:00Z",
...
| `data.contentRevisionId` | String | 양의 10진 문자열 수정본 식별자 |
| `data.contentId` | String | 양의 10진 문자열 원본 콘텐츠 식별자 |
| `data.status` | String | 항상 `EDIT_REQUESTED` |
| `data.editorId` | String | 양의 10진 문자열 소유 운영자 식별자 |
...
| `data.candidateImageUrlExpiresAt` | String | 후보 대표 이미지 조회 URL 만료 시각. UTC `Z` 문자열 |
| `data.submittedAt` | String | 생성과 동시에 심사 요청된 수정본의 동결 시각. UTC `Z` 문자열 |

### Error Code

| HTTP Status | Code | Description |
| --- | --- | --- |
| `400` | `INVALID_INPUT` | `revisionId`가 양의 10진 문자열이 아니다. 수정본과 콘텐츠 상태는 변경하지 않으며 요청 값을 수정한 뒤 재시도할 수 있다. |
| `400` | `INVALID_TYPE` | `revisionId`를 양의 10진 문자열 식별자로 해석할 수 없다. 수정본과 콘텐츠 상태는 변경하지 않으며 값 형식을 수정한 뒤 재시도할 수 있다. |
