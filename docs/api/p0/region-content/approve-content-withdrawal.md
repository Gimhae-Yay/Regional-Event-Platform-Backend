...
| --- | --- | --- |
| Base URL·미디어 타입·시간 형식 | [API 공통 규칙](../../common/api-conventions.md) | 실제 경로는 `/api/v1/region-admin/contents/{contentId}/withdraw`; 시각은 ISO 8601 `+09:00` 오프셋 문자열 |
...
| --- | --- | --- | --- |
| `contentId` | Long | Y | 철회 승인할 콘텐츠 식별자. 양의 정수여야 한다. |
...
```json
{
  ...
  "data": {
    "contentId": 101,
    "status": "WITHDRAWN",
    "withdrawnAt": "2026-07-30T10:00:00+09:00",
...
| `data.contentId` | Long | 철회된 콘텐츠 식별자 |
| `data.status` | String | 항상 `WITHDRAWN` |
| `data.withdrawnAt` | String | `WITHDRAWN` 상태 로그의 처리 시각 |
...
| --- | --- | --- |
| `400` | `INVALID_INPUT` | `contentId`가 양의 정수가 아니다. 콘텐츠·홀드·로그·감사 기록을 변경하지 않으며 요청 값을 수정한 뒤 재시도할 수 있다. |
