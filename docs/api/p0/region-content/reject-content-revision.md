...
| `409` | `CONTENT_STATE_CONFLICT` | 수정본이 `EDIT_REQUESTED`가 아니거나 원본 상태·후보 `publish_at` 조건이 맞지 않거나, 승인·철회·자동 공개·콘텐츠 중단·종료에 따른 `EDIT_INVALIDATED` 전이가 먼저 종결됐다. |
...
6. 승인·반려·철회·자동 공개·콘텐츠 중단·종료가 경합하면 현재 수정본·원본 상태를 조건으로 먼저 커밋한 하나만 성공한다. 콘텐츠 중단·종료가 먼저 `EDIT_INVALIDATED`를 커밋하면 이 요청은 `409 CONTENT_STATE_CONFLICT`로 거부한다.
