...
| Code | HTTP Status | 공개 메시지 | 발생 조건 |
| --- | --- | --- | --- |
| `SESSION_NOT_CANCELLABLE` | `409 Conflict` | 취소할 수 없는 회차 상태입니다. | 소유 운영자가 `SCHEDULED`가 아닌 회차의 취소를 요청한다. |
...
도메인 오류는 같은 `ErrorCode` enum에 추가한다. 오류 조건에는 상태 변경·미변경 여부와 재시도 가능 여부를 함께 작성한다.
