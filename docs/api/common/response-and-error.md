...
| 400 | `QR_VERIFICATION_FAILED` | QR을 확인할 수 없습니다. | QR 형식·버전·키 식별자·서명·만료 또는 서명된 예약·회차 참조 검증 실패 |
...
| 409 | `QR_ISSUE_CONFLICT` | QR을 발급할 수 없는 상태입니다. | 예약·회원 연결·회차 상태 또는 체크인 창 조건으로 단기 QR을 발급할 수 없는 상태 |
